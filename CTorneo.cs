...
namespace CTORNEO_FUTBOL
{
    ...
    public CTorneo()
        ...
        {
            string datos;
            if (this.ListaEquipos.Count == 0) { return datos = "NO HAY EQUIPOS REGISTRADOS"; }
            datos = "Equipos: \n\n";
            foreach (CEquipo equipo in ListaEquipos)
            {
        ...
        {
            string datos;
            if (ListaEquipos.Count == 0) { return datos = "NO HAY EQUIPOS ANOTADOS"; }
            datos = "Equipos aptos: \n\n";
            foreach (CEquipo equipo in ListaEquipos)
            {
        ...
        {
            string datos;
            if (ListaJugadores.Count == 0) { return datos = "NO HAY JUGADORES ANOTADOS"; }
            datos = "JUGADORES";
            ...
            foreach(CJugador jugador in ListaJugadores)
            {
                ...
                foreach (CEquipo equipo in ListaEquipos)
                {
                    if(equipo.BuscarJugador(jugador.GetDni()) != null)
                    {
        ...
        {
            string datos;
            if (ListaEntrenadores.Count == 0) { return datos = "NO HAY ENTRENADORES ANOTADOS"; }
            datos = "ENTRENADORES";
            ...
            foreach (CEntrenador entrenador in ListaEntrenadores)
            {
                ...
                foreach (CEquipo equipo in ListaEquipos)
                {
                    if (equipo.GetEntrenador() == entrenador )
                    {
                        datos += "\nEquipo: " + equipo.GetNombre();
                    }
                }
